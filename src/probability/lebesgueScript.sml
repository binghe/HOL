...
open HolKernel Parse boolLib bossLib arithmeticTheory realTheory prim_recTheory
     ...
     rich_listTheory pairTheory combinTheory realLib optionTheory pairTheory
     real_sigmaTheory jrhUtils;

open hurdUtils util_probTheory extrealTheory measureTheory borelTheory;

val _ = new_theory "lebesgue"; (* better be "lebesgue_integral" *)
  ...
  ``!m f (s :num->bool) a x
   ...
   >- (RW_TAC std_ss [FUN_EQ_THM]
       ...
       >> (MP_TAC o Q.SPEC `((\i. Normal (x (FST ((p :num -> num # num) i))) *
           ...
           o UNDISCH o Q.ISPEC `(s :num -> bool)CROSS(s' :num -> bool)`) EXTREAL_SUM_IMAGE_IN_IF
       >> RW_TAC std_ss []
       ...
       >> (MP_TAC o Q.SPEC `(\x'. Normal (x (FST x')) * indicator_fn ((\(i,j). a i INTER b j) x') t)`
           o UNDISCH o Q.ISPEC `(s :num -> bool)CROSS(s' :num -> bool)`) (GSYM EXTREAL_SUM_IMAGE_IN_IF)
       >> RW_TAC std_ss []
   ...
   >- (RW_TAC std_ss [pos_simple_fn_integral_def] >> FULL_SIMP_TAC std_ss [pos_simple_fn_def]
       ...
       >> `INJ p' (s CROSS s')
           (IMAGE p' (s CROSS s'))`
               by METIS_TAC [INJ_IMAGE_BIJ, BIJ_DEF]
       >> (MP_TAC o Q.SPEC `(\i:num. Normal (x (FST (p i))) * measure m ((\(i:num,j:num). a i INTER b j) (p i)))`
           o UNDISCH o Q.SPEC `p'` o UNDISCH o Q.SPEC `s CROSS s'`
           o INST_TYPE [alpha |-> ``:num#num``, beta |-> ``:num``]) EXTREAL_SUM_IMAGE_IMAGE
       ...
       >> RW_TAC std_ss []
       >> (MP_TAC o Q.SPEC `((\i. Normal (x (FST ((p :num -> num # num) i))) * measure m ((\(i,j). a i INTER b j) (p i))) o p')`
           o UNDISCH o Q.ISPEC `(s :num -> bool)CROSS(s' :num -> bool)`) EXTREAL_SUM_IMAGE_IN_IF
  ...
  >- ((MP_TAC o Q.SPEC `(\i. Normal (y i) * indicator_fn (b i) x')` o UNDISCH o
      ...
      >> `!x. SIGMA ((\x j. Normal (y x) * indicator_fn (a j INTER b x) x') x) s <> NegInf`
       ...
       >> (MP_TAC o Q.SPEC `(\x j. Normal (y x) * indicator_fn (a j INTER b x) x')`
           o Q.ISPECL [`s':num->bool`,`s:num->bool`]) EXTREAL_SUM_IMAGE_EXTREAL_SUM_IMAGE
       ...
       >> Suff `!x'''. x''' IN (s CROSS s') ==>
                       ((\x. Normal (y (SND x)) * indicator_fn (a (FST x) INTER b (SND x)) x') x''' =
                        (\x''. if x'' IN s CROSS s' then
                                 Normal (max (x (FST x'')) (y (SND x''))) * indicator_fn ((\ (i,j). a i INTER b j) x'') x'
                               else 0) x''')`
       >- (RW_TAC std_ss []
...
val pos_simple_fn_integral_indicator = store_thm
  ...
  >> (MP_TAC o Q.SPEC `0:num` o REWRITE_RULE [FINITE_SING]
      o Q.ISPECL [`(\i:num. Normal (if i = 0 then 0 else 1) * measure m (if i = 0 then m_space m DIFF A else A))`,`{1:num}`])
         EXTREAL_SUM_IMAGE_PROPERTY
...
val pos_simple_fn_integral_cmul = store_thm
  ...
  ``!m f s a x z. measure_space m /\ pos_simple_fn m f s a x /\ 0 <= z ==>
...
val integrable_eq = store_thm (* added by Chun Tian *)
  ("integrable_eq",
...
val integrable_zero = store_thm
  ("integrable_zero",
  ``!m c. measure_space m ==> integrable m (\x. 0)``,
    RW_TAC std_ss []
    ...
    `!x. 0 <= fn_plus f x + fn_plus g x` by METIS_TAC [FN_PLUS_POS, le_add] \\
     ...
     [ (* goal 1 (of 4) *)
      ...
      `f x + g x = fn_plus f x - fn_minus f x + (fn_plus g x - fn_minus g x)`
         by PROVE_TAC [FN_decomposition] >> POP_ORW \\
...
(* added `integrable m f` into antecedents, otherwise `integral m f` is not defined;
   added `measure m (m_space m) < PosInf` into antecedents (fine with prob_measure) *)
val finite_space_integral_reduce = store_thm
 ...
 >> Suff `measure m (PREIMAGE f {Normal (c x)} ∩ m_space m) <> NegInf`
 >- METIS_TAC [mul_not_infty]
 >> MATCH_MP_TAC pos_not_neginf
 >> IMP_RES_TAC MEASURE_SPACE_POSITIVE
 >> METIS_TAC [positive_def]);

(* More general case: `(m_space m)` can be infinite but `IMAGE f (m_space)` is finite.
...
(* No need to have PREIMAGE in this case *)
...
(* already defined in "src/n-bit/wordsScript.sml" *)
...
(* MEMO on the statements of Radon-Nikodym theorem:

1. from HVG's lebesgue_hvgScript.sml and HOL's original measureScript.sml

val Radon_Nikodym = store_thm (* -> "Radon_Nikodym_finite" *)
  ("Radon_Nikodym",
  ``!m v. measure_space m /\ measure_space v /\ (m_space v = m_space m) /\
         (measurable_sets v = measurable_sets m) /\
          measure v (m_space v) <> PosInf /\
...
2. from HOL's original measureScript.sml, without `measure m (m_space m) <> PosInf`

val Radon_Nikodym2 = store_thm (* -> "Radon_Nikodym" *)
  ("Radon_Nikodym2",
  ``!m v. measure_space m /\ measure_space v /\ (m_space v = m_space m) /\
         (measurable_sets v = measurable_sets m) /\
          measure_absolutely_continuous v m ==>
...
3. from HVG's lebesgue_measure_hvgScript.sml, with sigma finiteness

val RADON_NIKODYM = store_thm ("RADON_NIKODYM", (* -> Radon_Nikosym_sigma_finite *)
...
definition density :: "'a measure ==> ('a ==> ennreal) ==> 'a measure" where
  "density M f = measure_of (space M) (sets M) (\A. integral up+ x. f x * indicator A x d M)"
...
definition RN_deriv :: "'a measure ==> 'a measure ==> 'a ==> ennreal" where
  "RN_deriv M N =
    (if ?f. f IN borel_measurable M /\ density M f = N
       then SOME f. f IN borel_measurable M /\ density M f = N
       else (\_. 0))"

5. From [1]: Let u, v be two measures on the measurable space (X, AA). If u is sigma-finite,
