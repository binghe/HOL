...
(* c.f. "borel_space_def" of "old_borelScript.sml" *)
...
val half_open_interval_def = Define (* [a, b) *)
   `half_open_interval (a,b) = {x | Normal a <= x /\ x < Normal b}`;

val generator_def = Define
   `generator = (univ(:extreal), IMAGE half_open_interval univ(:real # real))`;
...
val LAMBDA0 = store_thm
  ("LAMBDA0",
  ``!a b. a <= b ==> (lambda0 (half_open_interval (a,b)) = Normal b - Normal a)``,
    cheat);

val SEMIRING_GENERATOR = store_thm
  ("SEMIRING_GENERATOR", ``semiring generator``,
    cheat);

(* need to use COMPACT_IMP_HEINE_BOREL *)
val PREMEASURE_LAMBDA0 = store_thm
  ("PREMEASURE_LAMBDA0",
  ``premeasure (univ(:extreal), IMAGE half_open_interval univ(:real # real),
                lambda0)``,
    cheat);

val SIGMA_GENERATOR_IS_BOREL = store_thm
  ("SIGMA_GENERATOR_IS_BOREL", ``sigma (space generator) (subsets generator) = Borel``,
    cheat);
