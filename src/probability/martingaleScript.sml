...
(* NOTE: removed “connected s”, replaced old antecedents:
   ...
   by new (weaker) antecedents:

      (!t. t IN s ==>
           ?a b w. t IN interval (a,b) /\ interval [a,b] SUBSET s /\
                   integrable m w /\
                  (!x. x IN m_space m ==> 0 <= w x /\ w x <> PosInf) /\
                   !c x. c IN interval (a,b) /\ x IN m_space m ==>
                         Normal (abs (diff1 (\t. u t x) c)) <= w x)
 *)
Theorem differentiable_lemma_alt :
    !s m u. measure_space (m :'a m_space) /\ open s /\
      ...
      (!x. x IN m_space m ==> (\t. u t x) differentiable_on s) /\
      (!t. t IN s ==>
           ?a b w. a < b /\ t IN interval (a,b) /\ interval [a,b] SUBSET s /\
                   integrable m w /\
                  (!x. x IN m_space m ==> 0 <= w x /\ w x <> PosInf) /\
                   !c x. c IN interval (a,b) /\ x IN m_space m ==>
                         Normal (abs (diff1 (\t. u t x) c)) <= w x)
...
Proof
 ...
 >> ‘!x. x IN m_space m ==> (\t. u t x) continuous_on s’
      by METIS_TAC [DIFFERENTIABLE_IMP_CONTINUOUS_ON]
 >> Q.PAT_X_ASSUM ‘!t. t IN s ==> ?a b w. _’ (MP_TAC o Q.SPEC ‘t’)
 >> simp [] >> STRIP_TAC (* this asserts a, b, and w *)
 >> Know ‘interval (a,b) SUBSET s’
 >- (Q_TAC (TRANS_TAC SUBSET_TRANS) ‘interval [a,b]’ >> art [] \\
     rw [SUBSET_INTERVAL])
 >> DISCH_TAC
 (* eliminating ‘diff1’ *)
 >> Q.PAT_X_ASSUM ‘!x. x IN m_space m ==> _ differentiable_on s’ MP_TAC
 ...
 >> Rewr'
 (* stage work *)
 >> Know ‘!c x. c IN interval (a,b) /\ x IN m_space m ==>
                Normal (abs (g c x)) <= w x’
 >- (rpt STRIP_TAC \\
     Q.PAT_X_ASSUM ‘!t x. t IN s /\ x IN m_space m ==> _’
       (MP_TAC o Q.SPECL [‘c’, ‘x’]) >> simp [] \\
     impl_tac >- METIS_TAC [SUBSET_DEF] \\
     DISCH_THEN (REWRITE_TAC o wrap o SYM) \\
     FIRST_X_ASSUM MATCH_MP_TAC >> art [])
 >> POP_ASSUM K_TAC (* diff1 (\t. u t x) t = g t x *)
 >> Q.PAT_X_ASSUM ‘!c x. _ ==> Normal (abs (diff1 (\t. u t x) c)) <= _’ K_TAC
 >> DISCH_TAC
 (* stage work *)
 >> Q.PAT_ASSUM ‘!t x. _ ==> (_ has_vector_derivative _) (at t within s)’
      (MP_TAC o Q.SPEC ‘t’)
 ...
 >- (rw [LIM_WITHIN, dist] \\
     ...
     [ (* goal 1 (of 2) *)
       ...
       shared_tactics () ])
 >> Rewr'
 (* applying LIM_WITHIN_OPEN *)
 >> Know ‘!f (l :real). (f --> l) (at t within s) <=>
                        (f --> l) (at t within interval (a,b))’
 >- (rpt GEN_TAC \\
     MATCH_MP_TAC LIM_WITHIN_OPEN_CONG \\
     simp [OPEN_INTERVAL])
 >> Rewr'
 (* stage work *)
 >> simp [LIM_WITHIN_SEQUENTIALLY]
  ...
  *)
 >> CONJ_ASM1_TAC
 >- (MP_TAC (Q.SPEC ‘interval (a,b)’ OPEN_CONTAINS_CBALL) \\
     simp [OPEN_INTERVAL] \\
     DISCH_THEN (MP_TAC o Q.SPEC ‘t’) \\
     simp [SUBSET_DEF, IN_CBALL] >> STRIP_TAC \\
    ...
    ‘!i. abs (h (SUC i)) < e’ by rw [ABS_REDUCE] \\
     ...
     >- (Q.X_GEN_TAC ‘n’ \\
         FIRST_X_ASSUM MATCH_MP_TAC >> fs [IN_CBALL]) >> DISCH_TAC \\
     qabbrev_tac ‘gi = \i x. inv (d (h1 i)) * (u (h1 i) x - u t x)’ \\
    ‘!x. x IN m_space m ==> ((\i. gi i x) --> g t x) sequentially’
       by rw [Abbr ‘gi’] \\
     Know ‘!i. integrable m (\x. Normal (gi i x))’
     >- (Q.X_GEN_TAC ‘n’ \\
         simp [Abbr ‘gi’, GSYM extreal_mul_eq, GSYM extreal_sub_eq] \\
  ...
  (* applying lebesgue_dominated_convergence *)
     ...
     >- (Q.EXISTS_TAC ‘w’ >> rw [extreal_abs_def] \\
      (* applying MVT_GENERAL_ALT *)
         Q.PAT_X_ASSUM ‘!x. x IN m_space m ==> (\t. u t x) continuous_on s’
           (MP_TAC o Q.SPEC ‘x’) >> rw [] \\
         Q.PAT_X_ASSUM ‘!c x. c IN s' /\ x IN m_space m ==> _’
           (MP_TAC o Q.SPEC ‘x’ o SIMP_RULE bool_ss [Once SWAP_FORALL_THM]) \\
         rw [] \\
         qabbrev_tac ‘u0 = (\t. u t x)’ \\
         qabbrev_tac ‘g0 = (\t. g t x)’ >> fs [] \\
        ...
        ‘t < t'’ by simp [Abbr ‘t'’, Abbr ‘h1’] \\
         ...
         >- (CONJ_TAC
             >- (MATCH_MP_TAC CONTINUOUS_ON_SUBSET \\
                 Q.EXISTS_TAC ‘s’ >> art [] \\
                 Q_TAC (TRANS_TAC SUBSET_TRANS) ‘s'’ >> art [] \\
                 simp [SUBSET_DEF, IN_INTERVAL] \\
                 Q.X_GEN_TAC ‘y’ >> STRIP_TAC \\
                 FIRST_X_ASSUM MATCH_MP_TAC \\
                 simp [dist, ABS_BOUNDS] \\
                 reverse CONJ_TAC
             ...
             Q.EXISTS_TAC ‘s’ >> simp [] \\
             CONJ_ASM1_TAC
             >- (Suff ‘y IN s'’ >- METIS_TAC [SUBSET_DEF] \\
                 FIRST_X_ASSUM MATCH_MP_TAC \\
                 simp [dist, ABS_BOUNDS] \\
                 reverse CONJ_TAC
                 >- (MATCH_MP_TAC REAL_LT_IMP_LE \\
 ...
 (* stage work *)
 >> Q.X_GEN_TAC ‘h’
 >> RW_TAC std_ss [Abbr ‘c’, Abbr ‘f’, Abbr ‘k’, FORALL_AND_THM]
 >> Q.PAT_X_ASSUM ‘!h x. x IN m_space m /\ _ /\ (h --> t) sequentially ==> _’
                  (MP_TAC o Q.SPEC ‘h’) >> rw []
 >> qabbrev_tac ‘q = Normal o g t’
 >> qabbrev_tac ‘qi = \i x. Normal (inv (d (h i)) * (u (h i) x - u t x))’
 >> Know ‘!x. x IN m_space m ==>
 ...
 >- (rw [Abbr ‘q'’, FUN_EQ_THM, Abbr ‘qi’, GSYM extreal_mul_eq] \\
     ...
     >- (simp [o_DEF] \\
         ...
         DISCH_THEN MATCH_MP_TAC \\
         METIS_TAC [SUBSET_DEF]) >> Rewr' \\
     qmatch_abbrev_tac ‘c * (real r1 - real r2) = _’ \\
    ‘h i IN s’ by METIS_TAC [SUBSET_DEF] \\
    ‘r1 <> PosInf /\ r1 <> NegInf /\ r2 <> PosInf /\ r2 <> NegInf’
       by METIS_TAC [integrable_finite_integral] \\
  (* applying mul_real *)
     Know ‘real (Normal c * (r1 - r2)) = real (Normal c) * real (r1 - r2)’
     >- (MATCH_MP_TAC mul_real >> simp [] \\
         METIS_TAC [sub_not_infty]) >> Rewr' \\
     Know ‘real (r1 - r2) = real r1 - real r2’
     >- (MATCH_MP_TAC sub_real >> art []) >> Rewr' \\
     simp [])
 >> Rewr'
 >> qunabbrev_tac ‘q'’
 ...
 >> Q.PAT_X_ASSUM ‘!x. x IN m_space m ==> (\t. u t x) continuous_on s’
      (MP_TAC o Q.SPEC ‘x’)
 >> Q.PAT_X_ASSUM ‘!c x. c IN interval (a,b) /\ x IN m_space m ==> _’
      (MP_TAC o Q.SPEC ‘x’ o SIMP_RULE bool_ss [Once SWAP_FORALL_THM])
 >> qabbrev_tac ‘u0 = (\t. u t x)’
 >> qabbrev_tac ‘g0 = (\t. g t x)’
 >> RW_TAC std_ss []
 ...
 >| [ (* goal 1 (of 2) *)
   ...
   (* applying MVT_GENERAL_ALT *)
      ...
      Know ‘interval [t',t] SUBSET s’
      >- (Q_TAC (TRANS_TAC SUBSET_TRANS) ‘interval (a,b)’ >> art [] \\
          simp [Once IN_INTERVAL, SUBSET_DEF] \\
          Q.X_GEN_TAC ‘z’ >> STRIP_TAC \\
       (* applying CONNECTED_IVT *)
          MATCH_MP_TAC CONNECTED_IVT \\
          qexistsl_tac [‘t'’, ‘t’] >> simp [Abbr ‘t'’] \\
          REWRITE_TAC [CONNECTED_INTERVAL]) >> DISCH_TAC \\
      impl_tac
      >- (CONJ_TAC
          >- (MATCH_MP_TAC CONTINUOUS_ON_SUBSET \\
              Q.EXISTS_TAC ‘s’ >> art []) \\
          ...
          Q.EXISTS_TAC ‘s’ >> art [] \\
          CONJ_ASM1_TAC
          >- (Suff ‘z IN interval (a,b)’ >- METIS_TAC [SUBSET_DEF] \\
              MATCH_MP_TAC CONNECTED_IVT \\
              qexistsl_tac [‘t'’, ‘t’] >> simp [REAL_LT_IMP_LE, Abbr ‘t'’] \\
...
Proof
    rpt GEN_TAC >> STRIP_TAC
 >> MP_TAC (Q.SPECL [‘s’, ‘m’, ‘u’] differentiable_lemma_alt) >> simp []
 >> DISCH_TAC
 >> CONJ_TAC
 >- (rw [differentiable_on, differentiable_alt_has_vector_derivative] \\
     Q.PAT_X_ASSUM ‘!t. t IN s ==>
