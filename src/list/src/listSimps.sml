...
val APPEND_EVAL_CONV =
   PURE_REWRITE_CONV [listTheory.APPEND_NIL, listTheory.APPEND]
...
(*
   takes a list of the form ``(l ++ [x1;...;xn]) ++ [y1;...;ym]`` and converts
   it to ``l ++ [x1;...;xn;y1;...;ym]``
*)
...
fun APPEND_SIMPLE_LISTS_ASSOC_CONV t =
  ...
  in
     (REWR_CONV (GSYM listTheory.APPEND_ASSOC)
      THENC RAND_CONV APPEND_EVAL_CONV) t
  end
  handle HOL_ERR _ => raise UNCHANGED
...
val NORM_CONS_APPEND_CONV = NORM_CONS_APPEND_NO_EVAL_CONV THENC APPEND_EVAL_CONV

val GSYM_CONS_APPEND_CONV =
   PURE_REWRITE_CONV [GSYM (CONJUNCT2 listTheory.APPEND)]
...
local
   ...
   fun is_right_cons lL1 lL2 =
      (is_non_empty_list (last lL1)) andalso (is_non_empty_list (last lL2))
   fun is_right_same lL1 lL2 =
      ((length lL1 + length lL2) > 2) andalso (aconv (last lL1) (last lL2))
         ...
         *)
           ...
           val thm1 = if n1 = n2 then thm0' else
               let
                  ...
                  val split_thm =
                     GSYM (APPEND_EVAL_CONV (mk_append (L21_t, L22_t)))
               in
           ...
           val thm2a =
              PART_MATCH
                 (lhs o rand)
                 (CONJUNCT2 listTheory.APPEND_11_LENGTH) (rhs (concl thm1))
           val thm2b =
              CONV_RULE
                 ((RATOR_CONV o RAND_CONV)
                  (REWRITE_CONV [listTheory.LENGTH, prim_recTheory.INV_SUC_EQ]))
                 thm2a
           ...
           val thm3 =
              if turn
                 then CONV_RULE
                        ((RHS_CONV o RATOR_CONV o RAND_CONV) SYM_CONV) thm2
              else thm2
   ...
   fun is_left_cons lL1 lL2 =
      (is_non_empty_list (hd lL1)) andalso (is_non_empty_list (hd lL2))
   fun is_left_same lL1 lL2 =
      (length lL1 + length lL2 > 2) andalso (aconv (hd lL1) (hd lL2))
      ...
      if (is_left_same lL1 lL2) then
         ...
         let
           ...
           val thm0 = ((if turn then SYM_CONV else ALL_CONV) THENC
                           (EQ_CONV right_nil_intro_CONV)) l
                      handle UNCHANGED => REFL l
           val thm1 = if n1 = n2 then thm0 else
               let
           ...
           val thm2a =
              PART_MATCH (lhs o rand) (CONJUNCT1 listTheory.APPEND_11_LENGTH)
                         (rhs (concl thm1))
           val thm2b =
              CONV_RULE
                 ((RATOR_CONV o RAND_CONV)
...
in
   local
      val conv =
         EQ_CONV NORM_CONS_APPEND_NO_EVAL_CONV
         THENC LIST_EQ_SIMP_CONV___internal
         THENC REWRITE_CONV [listTheory.APPEND_eq_NIL, listTheory.NOT_CONS_NIL,
                             GSYM listTheory.NOT_CONS_NIL, listTheory.CONS_11]
   in
