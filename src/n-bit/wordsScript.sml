...
fun add_TeX_tokens n =
  List.app
    (fn (s, m) =>
      TexTokenMap.TeX_notation {hol = s, TeX = ("\\HOLToken" ^ m ^ "{}", n)})
...
val () = add_infixes 485 HOLgrammars.RIGHT
  [("??",  `words$word_xor`),
   ("~??", `words$word_xnor`)]

val () = add_infixes 482 HOLgrammars.RIGHT
  [("!!",  `words$word_or`),
   ("||",  `words$word_or`),
   ("~||", `words$word_nor`)]
...
val () = add_TeX_tokens 1
  [("!!", "Or"), ("||", "Or"), (Unicode.UChar.or, "Or"),
   ("??", "Eor"), (Unicode.UChar.xor, "Eor")]
...
val () = add_TeX_tokens 1
   [("<+", "Lo"), (Unicode.UChar.lo, "Lo"),
    (">+", "Hi"), (Unicode.UChar.hi, "Hi"),
    ("<=+", "Ls"), (Unicode.UChar.ls, "Ls"),
    (">=+", "Hs"), (Unicode.UChar.hs, "Hs")]
...
val () = add_TeX_tokens 1
  [("#<<", "Rol"), (Unicode.UChar.rol, "Rol"),
   ("#>>", "Ror"), (Unicode.UChar.ror, "Ror")]

val () = add_TeX_tokens 2
  [("<<", "Lsl"), (Unicode.UChar.lsl, "Lsl"),
   (">>", "Asr"), (Unicode.UChar.asr, "Asr")]

val () = add_TeX_tokens 3
  [(">>>", "Lsr"), (Unicode.UChar.lsr, "Lsr")]
...
Theorem dimindex_dimword_le_iso:
  dimindex (:'a) <= dimindex (:'b) <=> dimword (:'a) <= dimword (:'b)
Proof SRW_TAC [] [logrootTheory.LE_EXP_ISO, fcpTheory.dimindex_def, dimword_def]
QED

Theorem dimindex_dimword_lt_iso:
  dimindex (:'a) < dimindex (:'b) <=> dimword (:'a) < dimword (:'b)
      ...
      Q.PAT_X_ASSUM `!f i. P` (Q.SPECL_THEN [`f`,`i`] ASSUME_TAC)
        ...
        \\ ASM_SIMP_TAC std_ss [BITS_SUM,
             (GSYM o REWRITE_RULE [LESS_EQ_REFL] o
              Q.SPECL [`p`,`n + p`,`n`]) BIT_OF_BITS_THM]
...
val BITWISE_ONE_COMP_THM = Q.prove(
  `!wl a b. 0 < wl ==>
...
val word_extract_n2w = save_thm("word_extract_n2w",
  (SIMP_RULE std_ss [BITS_COMP_THM2, word_bits_n2w, w2w_n2w] o
   Q.SPECL [`h`,`l`,`n2w n`] o SIMP_RULE std_ss [FUN_EQ_THM]) word_extract_def)
...
val lem = GEN_ALL (MATCH_MP LESS_LESS_EQ_TRANS (CONJ
  ((REWRITE_RULE [SUC_SUB,EXP_1] o Q.SPECL [`b`,`b`,`n`]) BITSLT_THM)
  TWO_EXP_DIMINDEX))
...
val BIT_SET = save_thm("BIT_SET",
  (REWRITE_RULE [ADD_0] o Q.SPEC `0`) BIT_SET_lem)
...
val word_and_n2w_alpha = Q.prove(
...
val word_or_1comp_n2w_alpha2 = Q.prove(
  `!n m. ~(n2w n) || ~(n2w m) =
...
val INV_SUC_EQ_mod = LET_RULE (Q.prove(
  `!m n. let $== = ^equiv in
           (SUC m == SUC n) = (m == n)`,
...
val EQ_ADD_LCANCEL_mod = LET_RULE (Q.prove(
  `!m n p. let $== = ^equiv in
           (m + n == m + p) = (n == p)`,
  LET_TAC \\ Induct \\ ASM_REWRITE_TAC [ADD_CLAUSES,INV_SUC_EQ_mod]))

val WORD_NEG_mod = LET_RULE (Q.prove(
  `!n. let $== = ^equiv in
...
val WORD_SUB_NEG = save_thm("WORD_SUB_NEG",
  (GEN_ALL o REWRITE_RULE [WORD_SUB] o Q.SPEC `- v`) WORD_SUB_RNEG)
...
val lem = Q.prove(
  ...
  REPEAT STRIP_TAC
    \\ IMP_RES_TAC DA
    \\ POP_ASSUM (Q.SPEC_THEN `a` STRIP_ASSUME_TAC)
    \\ Q.EXISTS_TAC `q`
    \\ FULL_SIMP_TAC arith_ss [MOD_MULT_])
...
val BITS_SUM1 =
  (GEN_ALL o REWRITE_RULE [MULT_LEFT_1] o
   Q.INST [`a` |-> `1`] o SPEC_ALL) BITS_SUM

val lem = (GSYM o SIMP_RULE arith_ss [] o
  Q.SPECL [`p`,`SUC m - n MOD SUC m + p`,
  ...
  SIMP_TAC (bool_ss++boolSimps.LET_ss) [Once (GSYM ROR_MOD)]
    ...
    >| [
      ...
      RULE_ASSUM_TAC (REWRITE_RULE [NOT_LESS])
        \\ IMP_RES_TAC LESS_EQUAL_ADD
        \\ ASSUME_TAC (Q.SPECL [`m`,`n MOD SUC m`,`a`] BITSLT_THM)
...
val SPEC_LESS_EXP_SUC_MONO = Q.prove(
  `2 ** ^HB < 2 ** dimindex (:'a)`,
  SRW_TAC [][DIMINDEX_GT_0])

val SPLIT_2_EXP_WL = Q.prove(
...
val BITS_COMP_MSB = (SIMP_RULE arith_ss [] o
  Q.SPECL [`m`,`0`,`m - 1`,`0`]) BITS_COMP_THM

val SLICE_COMP_MSB = Q.prove(
  `!b n. ~(b = 0) ==> (SLICE b b n + SLICE (b - 1) 0 n = SLICE b 0 n)`,
   REPEAT STRIP_TAC
     \\ POP_ASSUM (fn th => REWRITE_TAC [(SIMP_RULE arith_ss [SUB_SUC1,th] o
          Q.SPECL [`b`,`b - 1`,`0`,`n`]) SLICE_COMP_THM]))

val MSB_THM1 = Q.prove(
...
val TWO_COMP_POS = Q.prove(
  ...
  Cases
    ...
    \\ ASM_SIMP_TAC bool_ss [Q.SPECL [`m`,`m`] BITS_THM,SUC_SUB,EXP_1,EXP,
         TIMES2,LESS_EQ_ADD_SUB,DIV_MULT_1] \\ numLib.REDUCE_TAC)

val TWO_COMP_NEG_lem = Q.prove(
...
val SUB_EQUAL_WORD_L_INT_MIN = Q.prove(
  ...
  RW_TAC bool_ss [WORD_EQ_SUB_RADD] \\ STRIP_ASSUME_TAC EXISTS_HB
    ...
    \\ SUBST1_TAC ((SYM o Q.SPEC `n`) n2w_mod)
...
Theorem WORD_GREATER:
  !a:'a word b. a > b <=> b < a
Proof Cases_on `^HB = 0` >| [LEM2_TAC,LEM1_TAC]
QED

Theorem WORD_GREATER_EQ:
  !a:'a word b. a >= b <=> b <= a
Proof Cases_on `^HB = 0` >| [LEM2_TAC,LEM1_TAC]
QED

Theorem WORD_NOT_LESS:
  !a:'a word b. ~(a < b) <=> b <= a
Proof Cases_on `^HB = 0` >| [LEM2_TAC,LEM1_TAC]
QED
...
val WORD_GT = save_thm("WORD_GT",
  (Q.GEN `a` o Q.GEN `b` o REWRITE_CONV [WORD_GREATER,WORD_LT,GSYM GREATER_DEF])
  ``a:'a word > b``)
...
val WORD_GE = save_thm("WORD_GE",
  (Q.GEN `a` o Q.GEN `b` o
   REWRITE_CONV [WORD_GREATER_EQ,WORD_LE,GSYM GREATER_EQ]) ``a:'a word >= b``)
  ...
  RW_TAC bool_ss [word_lo] \\ Cases_on `b = 0w`
    \\ ASM_SIMP_TAC arith_ss [w2n_2comp,w2n_0,GSYM LESS_EQ_ADD_SUB,
         REWRITE_RULE [dimword_def]
                      (MATCH_MP LESS_IMP_LESS_OR_EQ (Q.SPEC `b` w2n_lt))]
    ...
    \\ RULE_ASSUM_TAC (REWRITE_RULE [GSYM w2n_11,w2n_0,w2n_n2w]) >| [
      ...
      RULE_ASSUM_TAC (REWRITE_RULE [NOT_LESS])
        \\ ASSUME_TAC (REWRITE_RULE [dimword_def] (Q.SPEC `a` w2n_lt))
...
Theorem WORD_LS:
  !a b. a <=+ b <=> w2n a <= w2n b
Proof PROVE_TAC [w2n_11,WORD_LO,WORD_LS_LO_EQ,LESS_OR_EQ]
QED

Theorem WORD_HI: !a b. a >+ b <=> w2n a > w2n b
Proof REWRITE_TAC [WORD_HI_NOT_LS,WORD_LS,GSYM NOT_GREATER]
QED

Theorem WORD_HS:     !a b. a >=+ b <=> w2n a >= w2n b
Proof REWRITE_TAC [WORD_HS_NOT_LO,WORD_LO,DECIDE ``~(a < b) <=> a >= b:num``]
QED

(* ------------------------------------------------------------------------- *)

Theorem WORD_NOT_LESS_EQUAL:      !a:'a word b. ~(a <= b) <=> b < a
Proof PROVE_TAC [WORD_NOT_LESS]
QED

Theorem WORD_LESS_OR_EQ:     !a:'a word b. a <= b <=> a < b \/ (a = b)
Proof LEM1_TAC
QED

Theorem WORD_GREATER_OR_EQ:    !a:'a word b. a >= b <=> a > b \/ (a = b)
...
Theorem WORD_HIGHER_EQ:     !a b. a >=+ b <=> b <=+ a
Proof RW_TAC arith_ss [WORD_HS,WORD_LS]
QED

Theorem WORD_NOT_LOWER:     !a b. ~(a <+ b) <=> b <=+ a
Proof RW_TAC arith_ss [WORD_LO,WORD_LS]
QED

Theorem WORD_NOT_LOWER_EQUAL:      !a b. ~(a <=+ b) <=> b <+ a
Proof PROVE_TAC [WORD_NOT_LOWER]
QED

Theorem WORD_LOWER_OR_EQ:          !a b. a <=+ b <=> a <+ b \/ (a = b)
Proof REWRITE_TAC [LESS_OR_EQ,WORD_LS,WORD_LO,w2n_11]
QED

Theorem WORD_HIGHER_OR_EQ:         !a b. a >=+ b <=> a >+ b \/ (a = b)
...
val tac =
    ...
    by (Q.EXISTS_TAC `(n + n') MOD dimword (:'a)`
          \\ IMP_RES_TAC LESS_EQUAL_ADD
          \\ SRW_TAC [ARITH_ss] [ZERO_LT_dimword, ADD_MODULUS])
    \\ SRW_TAC [ARITH_ss] [ZERO_LT_dimword, ADD_MODULUS]

Theorem WORD_ADD_LEFT_LO:
  !b c a.
      a + b <+ c <=>
      if b <=+ c then
...
val INT_MIN_LT_dimword = Q.prove(
  `INT_MIN (:'a) < dimword (:'a)`,
  SRW_TAC [] [INT_MIN_def, dimword_def, DIMINDEX_GT_0])

Theorem WORD_MSB_INT_MIN_LS:
...
val WORD_ADD_LEFT_LO2 = save_thm("WORD_ADD_LEFT_LO2",
  (GEN_ALL o SIMP_RULE (arith_ss++boolSimps.CONJ_ss++boolSimps.LET_ss)
   ...
   Q.SPECL [`a`, `a`, `c`]) WORD_ADD_LEFT_LO)

Theorem WORD_ADD_LEFT_LS2 =
  (GEN_ALL o REWRITE_RULE [GSYM WORD_LOWER_OR_EQ] o
   ...
   Q.SPECL [`a`, `a`, `c`]) WORD_ADD_LEFT_LS;

Theorem WORD_ADD_RIGHT_LO2 =
  (GEN_ALL o SIMP_RULE (arith_ss++boolSimps.CONJ_ss++boolSimps.LET_ss)
     [WORD_LOWER_EQ_REFL, WORD_ADD_0, WORD_LO_word_0,
      WORD_LOWER_OR_EQ, WORD_NEG_EQ, Once WORD_LESS_NEG_RIGHT,
      DECIDE ``a \/ ~a /\ b <=> a \/ b``] o
   Q.SPECL [`a`, `a`, `c`]) WORD_ADD_RIGHT_LO;

Theorem WORD_ADD_RIGHT_LS2 =
  (GEN_ALL o REWRITE_RULE [GSYM WORD_LOWER_OR_EQ] o
   SIMP_RULE (arith_ss++boolSimps.CONJ_ss++boolSimps.LET_ss)
     [WORD_LOWER_EQ_REFL, WORD_ADD_0, WORD_0_LS,
      WORD_LOWER_OR_EQ, WORD_NEG_EQ, Once WORD_LESS_NEG_RIGHT,
      DECIDE ``a \/ ~a /\ b <=> a \/ b``] o
   Q.SPECL [`a`, `a`, `c`]) WORD_ADD_RIGHT_LS;

Theorem word_msb_neg:
...
Proof ORDER_WORD_TAC
QED

Theorem word_gt_n2w:
  !a b. (n2w a):'a word > n2w b <=>
        let sa = BIT ^HB a and sb = BIT ^HB b in
          (sa = sb) /\ a MOD dimword(:'a) > b MOD dimword(:'a) \/ ~sa /\ sb
Proof ORDER_WORD_TAC
QED
...
val word_concat_word_T = save_thm("word_concat_word_T",
  (REWRITE_RULE [word_join_word_T] o Q.SPECL [`- 1w`,`- 1w`]) word_concat_def)

val BIT0_CONV = SIMP_CONV std_ss [BIT0_ODD]

val extract_00 = Q.prove(
  ...
  SRW_TAC [fcpLib.FCP_ss]
    ...
    \\ SRW_TAC [fcpLib.FCP_ss]
         [DIMINDEX_GT_0, BIT0_CONV ``BIT 0 1``, BIT0_CONV ``BIT 0 0``,
          (SIMP_RULE std_ss [] o Q.SPECL [`i`,`0`]) BIT_B_NEQ, BIT_ZERO]
    \\ Cases_on `i < dimindex (:'a)`
    \\ SRW_TAC [fcpLib.FCP_ss] [])
...
val word_reverse_reverse = Q.prove(
...
val word_reverse_EQ_ONE = Q.prove(
...
val WORD_SET_INDUCT = save_thm("WORD_SET_INDUCT",
  ...
  (Q.INST_TYPE [`:'a`|->`:'a word`] FINITE_INDUCT))
...
val WORD_ZERO_LE_SUB_LEMMA = Q.prove(
