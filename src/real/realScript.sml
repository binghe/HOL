...
Theorem real_of_num = real_of_num;

Theorem REAL_0 = REAL_0;
Theorem REAL_1 = REAL_1;
...
Theorem real_sub = real_sub;
Theorem real_lte = real_lte;
Theorem real_gt = real_gt;
Theorem real_ge = real_ge;
Theorem real_div = real_div;
...
(* |- !x. x + 0 = x *)
Theorem REAL_ADD_RID[simp] = REAL_ADD_RID;

Theorem REAL_ADD_RINV[simp] = REAL_ADD_RINV;

(* |- !x. x * 1 = x *)
Theorem REAL_MUL_RID[simp] = REAL_MUL_RID;

Theorem REAL_MUL_RINV:
   !x. ~(x = 0) ==> (x * inv x = 1)
Proof
...
(* |- !x y z. (x + y) * z = x * z + y * z *)
Theorem REAL_RDISTRIB = REAL_RDISTRIB;

Theorem REAL_EQ_LADD[simp] = REAL_EQ_ADD_LCANCEL;

Theorem REAL_EQ_RADD[simp] = REAL_EQ_ADD_RCANCEL;

(* also known as REAL_EQ_ADD_LCANCEL_0 *)
Theorem REAL_ADD_LID_UNIQ:
   !x y. (x + y = y) <=> (x = 0)
Proof
  REAL_ARITH_TAC
QED
...
(* |- !x y. (x + y = 0) <=> (x = -y) *)
Theorem REAL_LNEG_UNIQ = REAL_LNEG_UNIQ;

(* |- !x y. (x + y = 0) <=> (y = -x) *)
Theorem REAL_RNEG_UNIQ = REAL_RNEG_UNIQ;

(* |- !x y. -(x + y) = -x + -y *)
Theorem REAL_NEG_ADD = REAL_NEG_ADD;

(* |- !x. 0 * x = 0 *)
Theorem REAL_MUL_LZERO[simp] = REAL_MUL_LZERO;

(* |- !x. x * 0 = 0 *)
Theorem REAL_MUL_RZERO[simp] = REAL_MUL_RZERO;

(* |- !x y. -(x * y) = -x * y *)
Theorem REAL_NEG_LMUL = REAL_NEG_LMUL;

(* |- !x y. -(x * y) = x * -y *)
Theorem REAL_NEG_RMUL = REAL_NEG_RMUL;

(* |- !x. --x = x *)
Theorem REAL_NEGNEG[simp] = REAL_NEG_NEG;

Theorem REAL_NEG_MUL2:
   !x y. ~x * ~y = x * y
Proof
  REAL_ARITH_TAC
QED
...
(* |- !x y. (x * y = 0) <=> (x = 0) \/ (y = 0) *)
Theorem REAL_ENTIRE[simp] = REAL_ENTIRE;

(* |- !x y z. x + y < x + z <=> y < z *)
Theorem REAL_LT_LADD[simp] = REAL_LT_LADD;

(* |- !x y z. x + z < y + z <=> x < y *)
Theorem REAL_LT_RADD[simp] = REAL_LT_RADD;

(* |- !x y. ~(x < y) <=> y <= x *)
Theorem REAL_NOT_LT = REAL_NOT_LT;

(* |- !x y. ~(x < y /\ y < x) *)
Theorem REAL_LT_ANTISYM = REAL_LT_ANTISYM;

(* |- !x y. x < y ==> ~(y < x) *)
Theorem REAL_LT_GT = REAL_LT_GT;

(* |- !x y. ~(x <= y) <=> y < x *)
Theorem REAL_NOT_LE = REAL_NOT_LE;

(* |- !x y. x <= y \/ y <= x *)
Theorem REAL_LE_TOTAL = REAL_LE_TOTAL;

(* |- !x y. x <= y \/ y < x *)
Theorem REAL_LET_TOTAL = REAL_LET_TOTAL;

(* |- !x y. x < y \/ y <= x *)
Theorem REAL_LTE_TOTAL = REAL_LTE_TOTAL;

(* |- !x. x <= x *)
Theorem REAL_LE_REFL[simp] = REAL_LE_REFL;

(* |- !x y. x <= y <=> x < y \/ (x = y) *)
Theorem REAL_LE_LT = REAL_LE_LT;

(* |- !x y. x < y <=> x <= y /\ x <> y *)
Theorem REAL_LT_LE = REAL_LT_LE;

(* |- !x y. x < y ==> x <= y *)
Theorem REAL_LT_IMP_LE = REAL_LT_IMP_LE;

(* |- !x y z. x < y /\ y <= z ==> x < z *)
Theorem REAL_LTE_TRANS = REAL_LTE_TRANS;

(* |- !x y z. x <= y /\ y < z ==> x < z *)
Theorem REAL_LET_TRANS = REAL_LET_TRANS;

(* |- !x y z. x <= y /\ y <= z ==> x <= z *)
Theorem REAL_LE_TRANS = REAL_LE_TRANS;

(* |- !x y. x <= y /\ y <= x <=> (x = y) *)
Theorem REAL_LE_ANTISYM = REAL_LE_ANTISYM;

Theorem REAL_LET_ANTISYM:
   !x y. ~(x < y /\ y <= x)
Proof
  REAL_ARITH_TAC
QED
...
(* |- !x y. ~(x <= y /\ y < x) *)
Theorem REAL_LTE_ANTISYM = REAL_LTE_ANTISYM;
...
Theorem REAL_NEG_GT0[simp]:
   !x. 0 < ~x <=> x < 0
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_NEG_LE0[simp]:
   !x. ~x <= 0 <=> 0 <= x
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_NEG_GE0[simp]:
   !x. 0 <= ~x <=> x <= 0
Proof
  REAL_ARITH_TAC
QED
...
(* |- !x y. 0 <= x /\ 0 <= y ==> 0 <= x * y *)
Theorem REAL_LE_MUL = REAL_LE_MUL;

(* |- !x. 0 <= x * x *)
Theorem REAL_LE_SQUARE = REAL_LE_SQUARE;

(* |- 0 <= 1 *)
Theorem REAL_LE_01 = REAL_LE_01;

(* |- 0 < 1 *)
Theorem REAL_LT_01 = REAL_LT_01;

(* |- !x y z. x + y <= x + z <=> y <= z *)
Theorem REAL_LE_LADD[simp] = REAL_LE_LADD;

(* |- !x y z. x + z <= y + z <=> x <= y *)
Theorem REAL_LE_RADD[simp] = REAL_LE_RADD;
...
(* |- !x y. 0 <= x /\ 0 <= y ==> 0 <= x + y *)
Theorem REAL_LE_ADD = REAL_LE_ADD;

(* |- !x y. 0 < x /\ 0 < y ==> 0 < x + y *)
Theorem REAL_LT_ADD = REAL_LT_ADD;

Theorem REAL_LT_ADDNEG:
   !x y z. y < x + ~z <=> y + z < x
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_LT_ADDNEG2:
   !x y z. (x + ~y) < z <=> x < (z + y)
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_LT_ADD1:
   !x y. x <= y ==> x < (y + &1)
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_SUB_ADD:
   !x y. (x - y) + y = x
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_SUB_ADD2:
   !x y. y + (x - y) = x
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_SUB_REFL[simp]:
   !x. x - x = 0
Proof
  REAL_ARITH_TAC
QED
...
(* |- !x y. (x - y = 0) <=> (x = y) *)
Theorem REAL_SUB_0[simp] = REAL_SUB_0;

Theorem REAL_LE_DOUBLE:
   !x. 0 <= x + x <=> 0 <= x
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_LE_NEGL:
   !x. (~x <= x) <=> (0 <= x)
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_LE_NEGR:
   !x. (x <= ~x) <=> (x <= 0)
Proof
  REAL_ARITH_TAC
QED
...
(* |- -0 = 0 *)
Theorem REAL_NEG_0[simp] = REAL_NEG_0;

(* |- !x y. -(x - y) = y - x *)
Theorem REAL_NEG_SUB = REAL_NEG_SUB;
...
Theorem REAL_SUB_RDISTRIB:
   !x y z. (x - y) * z = (x * z) - (y * z)
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_SUB_LZERO[simp]:
   !x. 0 - x = ~x
Proof REAL_ARITH_TAC
QED

Theorem REAL_SUB_RZERO[simp]:
   !x. x - 0 = x
Proof REAL_ARITH_TAC
QED

(* also known as REAL_EQ_MUL_LCANCEL *)
Theorem REAL_EQ_LMUL[simp]:
...
Theorem REAL_LT_IMP_NE:
   !x y. x < y ==> ~(x = y)
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_INV_POS:
   !x. 0 < x ==> 0 < inv x
Proof
...
(* !n. 0 <= &n *)
Theorem REAL_POS[simp] = REAL_POS;

(* !n. 0 < &SUC n *)
Theorem REAL_POS_LT = REAL_POS_LT;

(* |- !m n. &m <= &n <=> m <= n *)
Theorem REAL_LE[simp] = REAL_LE;

Theorem REAL_LT[simp]:
   !m n. &m < &n <=> m < n
Proof
...
Theorem REAL_INJ[simp] = REAL_INJ;

(* |- !m n. &m + &n = &(m + n) *)
Theorem REAL_ADD[simp] = REAL_ADD;

(* |- !m n. &m * &n = &(m * n) *)
Theorem REAL_MUL[simp] = REAL_MUL;
...
(* |- !n. &n <> 0 <=> 0 < &n *)
Theorem REAL_LT_NZ = REAL_LT_NZ;

Theorem REAL_NZ_IMP_LT:
   !n. ~(n = 0) ==> 0 < &n
Proof
  GEN_TAC THEN REWRITE_TAC[GSYM REAL_INJ, REAL_LT_NZ]
QED

Theorem REAL_LT_RDIV_0:
   !y z. 0 < z ==> (0 < (y / z) <=> 0 < y)
Proof
  REPEAT GEN_TAC THEN DISCH_TAC THEN
...
Theorem REAL_SUB_SUB:
   !x y. (x - y) - x = ~y
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_LT_ADD_SUB:
   !x y z. (x + y) < z <=> x < (z - y)
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_LT_SUB_RADD:
   !x y z. (x - y) < z <=> x < z + y
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_LT_SUB_LADD:
   !x y z. x < (y - z) <=> (x + z) < y
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_LE_SUB_LADD:
   !x y z. x <= (y - z) <=> (x + z) <= y
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_LE_SUB_RADD:
   !x y z. (x - y) <= z <=> x <= z + y
Proof
  REAL_ARITH_TAC
QED
...
(* |- !x y. -x <= -y <=> y <= x *)
Theorem REAL_LE_NEG[simp] = REAL_LE_NEG2;

Theorem REAL_ADD2_SUB2:
...
(* |- !x y. 0 <= x /\ 0 < y ==> 0 < x + y *)
Theorem REAL_LET_ADD = REAL_LET_ADD;

(* |- !x y. 0 < x /\ 0 <= y ==> 0 < x + y *)
Theorem REAL_LTE_ADD = REAL_LTE_ADD;
...
Theorem REAL_SUB_RNEG:
   !x y. x - ~y = x + y
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_SUB_NEG2:
   !x y. ~x - ~y = y - x
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_SUB_TRIANGLE:
   !a b c. (a - b) + (b - c) = a - c
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_EQ_SUB_LADD:
   !x y z. (x = y - z) = (x + z = y)
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_EQ_SUB_RADD:
   !x y z. (x - y = z) = (x = z + y)
Proof
  REAL_ARITH_TAC
QED
...
Theorem REAL_SUB_SUB2:
   !x y. x - (x - y) = y
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_ADD_SUB2:
   !x y. x - (x + y) = ~y
Proof
  REAL_ARITH_TAC
QED
...
Theorem REAL_EQ_IMP_LE:
   !x y. (x = y) ==> x <= y
Proof
  REAL_ARITH_TAC
QED

Theorem REAL_INV_LT1:
   !x. 0 < x /\ x < &1 ==> &1 < inv(x)
Proof
...
Theorem abs = real_abs; (* moved to realaxTheory *)
...
(* |- !x y. x * -y = -(x * y) *)
Theorem REAL_MUL_RNEG = REAL_MUL_RNEG;
 ...
 *)
Theorem real_lt = real_lt;

(* |- !x y z. y <= z ==> x + y <= x + z *)
Theorem REAL_LE_LADD_IMP = REAL_LE_LADD_IMP;
...
(* |- !x y. x <= -y <=> x + y <= 0 *)
Theorem REAL_LE_RNEG = REAL_LE_RNEG;

Theorem REAL_LT_RNEG:
    !x y. x < -y <=> x + y < &0:real
Proof
...
(* !x n. &x pow n = &(x ** n) *)
Theorem REAL_OF_NUM_POW = REAL_OF_NUM_POW;
...
Theorem REAL_EQ_MUL_LCANCEL:
    !x y z. (x * y = x * z) <=> (x = 0) \/ (y = z)
Proof
  REWRITE_TAC [REAL_EQ_LMUL]
QED

Theorem REAL_ABS_0[simp] = ABS_0;

Theorem REAL_ABS_TRIANGLE = ABS_TRIANGLE;

Theorem REAL_ABS_MUL = ABS_MUL;

Theorem REAL_ABS_POS = ABS_POS;

Theorem REAL_LE_EPSILON:
...
Theorem min_def = real_min; (* moved to realaxTheory *)

Theorem REAL_MIN_REFL:
     !x. min x x = x
Proof
...
Theorem max_def = real_max; (* moved to realaxTheory *)

Theorem REAL_MAX_REFL:
     !x. max x x = x
Proof
...
Theorem REAL_DIV_INNER_CANCEL2 =
   SIMP_RULE boolSimps.bool_ss [numLib.ARITH_PROVE ``~(2n = 0)``, REAL_INJ]
   (Q.SPEC `2` REAL_DIV_INNER_CANCEL);

Theorem REAL_DIV_INNER_CANCEL3 =
   SIMP_RULE boolSimps.bool_ss [numLib.ARITH_PROVE ``~(3n = 0)``, REAL_INJ]
   (Q.SPEC `3` REAL_DIV_INNER_CANCEL);

Theorem REAL_DIV_OUTER_CANCEL:
...
Theorem REAL_DIV_OUTER_CANCEL2 =
   SIMP_RULE boolSimps.bool_ss [numLib.ARITH_PROVE ``~(2n = 0)``, REAL_INJ]
   (Q.SPEC `2` REAL_DIV_OUTER_CANCEL);

Theorem REAL_DIV_OUTER_CANCEL3 =
   SIMP_RULE boolSimps.bool_ss [numLib.ARITH_PROVE ``~(3n = 0)``, REAL_INJ]
   (Q.SPEC `3` REAL_DIV_OUTER_CANCEL);

Theorem REAL_DIV_REFL2 =
   SIMP_RULE boolSimps.bool_ss [numLib.ARITH_PROVE ``~(2n = 0)``, REAL_INJ]
   (Q.SPEC `2` REAL_DIV_REFL);

Theorem REAL_DIV_REFL3 =
   SIMP_RULE boolSimps.bool_ss [numLib.ARITH_PROVE ``~(3n = 0)``, REAL_INJ]
   (Q.SPEC `3` REAL_DIV_REFL);

Theorem REAL_HALF_BETWEEN:
...
Theorem NUM_FLOOR_DIV_LOWERBOUND:
