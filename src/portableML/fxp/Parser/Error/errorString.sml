...
structure ErrorString : ErrorString =
   struct
      ...
      (* NOTE: the old name QUOTE is conflicted with HOL4's term parser. *)
      val quoteChar = "'"
      fun quoteErrorChar0 c = quoteChar ^ errorChar2String c ^ quoteChar
      fun quoteErrorChar c = if c=0wx0 then "entity end"
                             else quoteChar ^ errorChar2String c ^ quoteChar
      fun quoteErrorData cs = quoteChar ^ errorData2String cs ^ quoteChar
      fun quoteErrorString s = quoteChar ^ s ^ quoteChar
      fun quoteErrorVector v = quoteChar ^ errorVector2String v ^ quoteChar
