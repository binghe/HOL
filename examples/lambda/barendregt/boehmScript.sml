...
(* Definition 10.1.9 [1, p.221] (Effective Boehm Tree)
   ...
   NOTE: For the correctness of this definition, ‘X SUBSET FV M0’ or something
         equivalent must be guaranteed by the user.
 *)
...
Theorem BT_of_unsolvables :
    !X M r. unsolvable M ==> BTe X M r = bot
Proof
    rw [BT_def, BT_generator_def, ltree_unfold, ltree_map]
QED

Theorem BT_of_unsolvables_cong :
    !X M N r. unsolvable M /\ unsolvable N ==> BTe X M r = BTe X N r
Proof
    rw [BT_of_unsolvables]
QED

Theorem BT_of_principle_hnf :
    !X M r. solvable M ==> BTe X (principle_hnf M) r = BTe X M r
Proof
...
Theorem BT_finite_branching :
    !X M r. finite_branching (BTe X M r)
Proof
    rpt GEN_TAC
 >> irule finite_branching_coind'
 >> Q.EXISTS_TAC ‘\b. ?X M r. b = BTe X M r’
 >> rw [] >- (qexistsl_tac [‘X’, ‘M’, ‘r’] >> rw [])
 (* stage work *)
 >> qabbrev_tac ‘A = BTe X M r’
 ...
 >- rw [Abbr ‘A’, Abbr ‘ts’, BT_def, BT_generator_def, Once ltree_unfold,
        LFINITE_fromList]
 >> qabbrev_tac ‘P = \b. ?X M r. b = BTe X M r’
 (* stage work *)
...
Proof
 ...
 >> Q.EXISTS_TAC
     ‘\x y. ?P Q r. FV P UNION FV Q SUBSET (X UNION (FRESH_SET (SUC r) X)) /\
                    P == Q /\ x = BTe X P r /\ y = BTe X Q r’
 >> BETA_TAC
 ...
 >> Q.PAT_X_ASSUM ‘_ = BTe X Q r’ MP_TAC
 >> simp [BT_def, Once ltree_unfold, BT_generator_def]
 >> Q.PAT_X_ASSUM ‘_ = BTe X P r’ MP_TAC
 >> simp [BT_def, Once ltree_unfold, BT_generator_def]
 >> NTAC 2 STRIP_TAC
 ...
 >| [ (* goal 1 (of 2) *)
      ...
      >- (Know ‘x IN X UNION Y’ >- METIS_TAC [SUBSET_DEF] \\
          rw [] >- (DISJ1_TAC >> art []) \\
          DISJ2_TAC \\
          Suff ‘Y SUBSET FRESH_SET' (SUC r) X’ >- METIS_TAC [SUBSET_DEF] \\
          qunabbrev_tac ‘Y’ \\
          MATCH_MP_TAC FRESH_SET_MONO >> rw []) \\
      DISJ2_TAC \\
      Suff ‘set vs SUBSET FRESH_SET' (SUC r) X’ >- METIS_TAC [SUBSET_DEF] \\
 ...
 *)
Theorem subterm_imp_ltree_paths_lemma[local] :
...
Proof
 ...
 >> simp [Abbr ‘P’]
 >> CONV_TAC (UNBETA_CONV “BTe X M r”)
 >> qmatch_abbrev_tac ‘P _’
...
Proof
    rpt STRIP_TAC
 >> MATCH_MP_TAC subterm_imp_ltree_paths_lemma >> art []
...
(* ltree_lookup returns more information (the entire subtree), thus can be
   ...
   |- !r p X M.
        p IN ltree_paths (BTe X M r) ==>
        ltree_el (BTe X M r) p =
        do t' <- ltree_lookup (BTe X M r) p;
...
Theorem BT_subterm_lemma_general :
    !p X M r. FINITE X /\ FV M SUBSET X UNION FRESH_SET' r X /\
              subterm X M p r <> NONE ==>
              BT X (THE (subterm X M p r)) = THE (ltree_lookup (BTe X M r) p)
Proof
    Induct_on ‘p’
...
Proof
    rpt STRIP_TAC
 >> MATCH_MP_TAC BT_subterm_lemma_general >> art []
 >> Q.PAT_X_ASSUM ‘FV M SUBSET X’ MP_TAC
 >> SET_TAC []
QED

(* NOTE: This theorem is no more valid (and needed)
...
Theorem subterm_rank_lemma :
    !p X M N r r'. FINITE X /\ FV M SUBSET X UNION FRESH_SET' r X /\
                   subterm X M p r = SOME (N,r')
               ==> r' = r + LENGTH p /\
                   FV N SUBSET X UNION FRESH_SET' r' X
Proof
    Induct_on ‘p’
 >- (rw [] >> art [])
 >> rpt GEN_TAC
...
Proof
    rpt STRIP_TAC
 >> ‘p IN ltree_paths (BTe X M r)’ by PROVE_TAC [subterm_imp_ltree_paths]
 >> ‘ltree_lookup (BTe X M r) p <> NONE’ by rw [GSYM ltree_lookup_valid]
 >> Know ‘BT X (THE (subterm X M p r)) = THE (ltree_lookup (BTe X M r) p)’
 ...
 >> rw [BT_ltree_el_thm]
 >> Know ‘BTe X N r' = ltree_unfold (BT_generator X) (N,r')’ >- rw [BT_def]
 >> Rewr'
...
(* stage work *)
 ...
 >> ‘FV N SUBSET X UNION FRESH_SET' r' X’ by PROVE_TAC [subterm_rank_thm]
 >> MATCH_MP_TAC DISJOINT_SUBSET
 >> Q.EXISTS_TAC ‘X UNION FRESH_SET' r' X’ >> art []
...
Theorem BT_ltree_lookup_tpm :
    !p X Y M pi. FINITE X /\ FINITE Y /\
                 ...
                 ltree_lookup (BT Y (tpm pi M)) p <> NONE
Proof
 ...
 >> Know ‘BT X M = ltree_unfold BT_generator (X,M)’ >- rw [BT_def]
 >> simp [Once ltree_unfold, BT_generator_def, LNTH_fromList, LMAP_fromList]
 >> DISCH_THEN K_TAC (* BT X M = ... *)
 >> REWRITE_TAC [GSYM BT_def]
 >> Cases_on ‘h < LENGTH Ms’ >> simp [EL_MAP]
 (* stage work *)
 >> Know ‘BT Y (tpm pi M) = ltree_unfold BT_generator (Y,tpm pi M)’ >- rw [BT_def]
 >> simp [Once ltree_unfold, BT_generator_def, LNTH_fromList, LMAP_fromList]
 >> DISCH_THEN K_TAC (* BT Y (tpm pi M) = ... *)
 ...
 >> Q.PAT_X_ASSUM ‘ltree_lookup (BT (X UNION set vs) (EL h Ms)) p <> NONE’ MP_TAC
...
Proof
 ...
 >> ‘l IN ltree_paths (BT X M)’ by PROVE_TAC [ltree_paths_inclusive]
 >> simp []
...
Theorem BT_ltree_lookup_of_unsolvables :
    !p X M. FINITE X /\ subterm X M p <> NONE /\ unsolvable (subterm' X M p) ==>
            ltree_lookup (BT X M) p = SOME bot
Proof
    Induct_on ‘p’
...
Theorem subterm_subst_cong_lemma[local] :
    !q X M p. q <<= p /\ FINITE X /\ v IN X /\
          ...
          ==> subterm X ([P/v] M) q <> NONE /\
              tpm_rel (subterm' X ([P/v] M) q) ([P/v] (subterm' X M q))
Proof
    Induct_on ‘q’ >- rw []
 >> rpt GEN_TAC
 >> STRIP_TAC
 ...
 >- (Q.PAT_X_ASSUM ‘!q. q <<= FRONT (h::t) ==> solvable (subterm' X M q)’
       (MP_TAC o (Q.SPEC ‘[]’)) >> rw [])
 >> DISCH_TAC
 >> Know ‘subterm X M (h::q) <> NONE’
 >- (FIRST_X_ASSUM MATCH_MP_TAC >> rw [])
 (* BEGIN Norrish's advanced tactics *)
 >> CONV_TAC (UNBETA_CONV “subterm X M (h::q)”)
 >> qmatch_abbrev_tac ‘f _’
 >> RW_TAC bool_ss [subterm_of_solvables]
 >> simp [Abbr ‘f’]
 (* END Norrish's advanced tactics. *)
 >> STRIP_TAC
 (* get explicit representation of M0 *)
 >> Know ‘ALL_DISTINCT vs /\ DISJOINT (set vs) (X UNION FV M0) /\ LENGTH vs = n’
 >- rw [Abbr ‘vs’, NEWS_def]
 >> DISCH_THEN (STRIP_ASSUME_TAC o (REWRITE_RULE [DISJOINT_UNION']))
 >> Q_TAC (HNF_TAC (“M0 :term”, “vs :string list”,
  ...
  *)
 >> Know ‘subterm (X UNION set vs) (EL h args) t <> NONE /\
  ...
  (* applying subterm_width_alt *)
     ...
     >- (rw [Once EXTENSION] \\
         ...
         [ (* goal 1 (of 2) *)
           Q.EXISTS_TAC ‘p'’ >> rw [],
           (* goal 2 (of 2) *)
           Q.EXISTS_TAC ‘subterm' Y M p'’ >> art [] \\
           Q.EXISTS_TAC ‘p'’ >> art [] ]) >> Rewr' \\
     qunabbrev_tac ‘p’ \\
     Cases_on ‘t = []’ >- rw [] \\
 ...
 (* LHS rewriting of args', this will introduce M0' = principle_hnf ([P/v] M)
    ...
    NOTE: Until now the ‘X’ of ‘subterm X ([P/v] M) (h::l)’ can be anything.
  *)
 >> CONV_TAC (UNBETA_CONV “subterm X ([P/v] M) (h::q)”)
 >> qmatch_abbrev_tac ‘f _’
 >> ASM_SIMP_TAC std_ss [subterm_of_solvables]
 >> BasicProvers.LET_ELIM_TAC
 >> Q.PAT_X_ASSUM ‘subterm _ (EL h (hnf_children M1)) q <> NONE’ MP_TAC
 >> simp [Abbr ‘f’, hnf_children_hnf]
 >> DISCH_TAC (* subterm (X UNION set vs) (EL h args) q <> NONE *)
 ...
 (* NOTE: now we show that vs and vs' are generated from the same excluded list.
    ...
    disjointness information for xs and z. (‘y IN X’ plays an essential role.)
  *)
 ...
 >- (simp [FV_LAMl, FV_appstar] \\
     ...
     >- (qunabbrev_tac ‘Z’ \\
         ...
         Q.PAT_X_ASSUM ‘!N. MEM N args' ==> DISJOINT (FV N) (set ls)’
           (MP_TAC o (Q.SPEC ‘N’)) \\
 ...
 >> ‘FINITE Z’ by rw [Abbr ‘Z’]
 (* change all occurrences of Z to Y in the goal, new tpm_rel created *)
 >> Suff ‘subterm Y ([P/y] N) q <> NONE /\
          tpm_rel (subterm' Y ([P/y] N) q) ([P/y] (subterm' Y N q))’
 >- (STRIP_TAC \\
     CONJ_ASM1_TAC >- PROVE_TAC [subterm_tpm_cong] \\
  (* applying tpm_rel_cong and subterm_tpm_cong *)
     Suff ‘tpm_rel (subterm' Z ([P/y] N) q) ([P/y] (subterm' Y N q)) <=>
           tpm_rel (subterm' Y ([P/y] N) q) ([P/y] (subterm' Y N q))’ >- rw [] \\
...
(* NOTE: ‘y IN X’ must hold to make sure that ‘~MEM y vs’, where ‘set vs’ is disjoint
   ...
   The head variable ‘y’ of M0 (= LAMl vs (VAR y @* args)) must come from 2) or 3)
 *)
       ...
       ?pi. Boehm_transform pi /\ is_ready' (apply pi M) /\
           ?Z v P. Z = X UNION FV M /\ closed P /\
               !q. q <<= p /\ q <> [] ==>
                   ...
                   tpm_rel (subterm' Z (apply pi M) q) ([P/v] (subterm' Z M q))
Proof
    rpt STRIP_TAC
 >> ‘(!q. q <<= p ==> subterm X M q <> NONE) /\
      !q. q <<= FRONT p ==> solvable (subterm' X M q)’
 ...
 >> qunabbrev_tac ‘d’
 (* applying subterm_width_alt *)
 >> MP_TAC (Q.SPECL [‘X’, ‘M’, ‘p’] subterm_width_alt)
 >> simp [] >> DISCH_THEN K_TAC
 >> Know ‘IMAGE (hnf_children_size o principle_hnf)
                {subterm' X M p' | p' <<= FRONT p} =
          {hnf_children_size (principle_hnf (subterm' X M p')) | p' <<= FRONT p}’
...
(* >> DISCH_THEN (STRIP_ASSUME_TAC o Q.SPEC ‘k’) *)
 ...
 >> Know ‘!i. i < k ==> EL (j i) xs = b i /\ B i = VAR (b i) /\ j i < LENGTH xs’
 >- RW_TAC std_ss []
 >> NTAC 2 (POP_ASSUM K_TAC) >> DISCH_TAC
 ...
 >> Know ‘!i. i < k ==> P @* Ns i @@ VAR (b i) @* tl i -h->* VAR (b i) @* Ns i @* tl i’
 ...
 >> Know ‘!i. i < k ==> principle_hnf (apply pi (M i)) = VAR (b i) @* Ns i @* tl i’
 >- (rpt (Q.PAT_X_ASSUM ‘Boehm_transform _’ K_TAC) \\
  ...
  (* applying principle_hnf_ISUB_cong *)
    ...
    ‘MAP VAR vs :term list = TAKE (n i) (MAP VAR vs) ++ DROP (n i) (MAP VAR vs)’
       by rw [TAKE_DROP] \\
     POP_ASSUM
  ...
  (* applying principle_hnf_denude_thm, finally *)
     ...
     >- (MATCH_MP_TAC DISJOINT_SUBSET' \\
         ...
         DISJ2_TAC >> Q.EXISTS_TAC ‘FV (M i)’ >> art [] \\
