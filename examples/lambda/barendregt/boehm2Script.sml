...
(* Proposition 10.1.6 [1, p.219] (beta-equivalent terms have the same Boehm tree)
   ...
   NOTE3: this theorem doesn't hold with latest version of BT_generator_def
...
Overload subterm' = “\X M p. THE (subterm X M p)”
...
Proof
 ...
 >> CONV_TAC (UNBETA_CONV “subterm X M (h::p)”)
 >> qmatch_abbrev_tac ‘P _’
 >> reverse (RW_TAC bool_ss [subterm_def])
 >> simp [Abbr ‘P’]
 (* unsolvable M *)
...
(* NOTE: ‘MEM v vs’ is a difficult situation in this lemma, as we have no control
   over both ‘v’ and ‘vs’.

   Going back to the caller theorem, M0 = LAMl vs M1 = LAMl vs (VAR y @* args),
   where FV M1 SUBSET (X UNION FV M0 UNION set vs). The "M" here is one element
   of args. If y # M, [P/v] M = M, and LHS simply becomes ‘subterm' X M l’. But
   RHS ‘subterm' X M t’ may have ‘v’ as new free variables (coming from closed
   binding list, and then [P/v] will substitte them with P.  The only reliable
   way to make sure ‘v’ doesn't occur in any new binding list, is to have
...
Proof
 ...
 >- (simp [] >> DISCH_TAC \\
  ...
  (* now applying IH *)
     ...
     FIRST_X_ASSUM MATCH_MP_TAC \\
     Q.EXISTS_TAC ‘t’ >> simp [] \\
  (* t IN ltree_paths (BTe X N) /\ subterm X N t <> NONE /\ subterm_width N t <= d *)
     cheat)
 (* Case 3 *)
 >> Q.PAT_X_ASSUM ‘s = s' \/ s = v INSERT s'’ MP_TAC
...
(* Lemma 10.3.6 (ii) [1, p.247]:
   ...
   NOTE4: The precise forms of Y is ‘X UNION FV M’, if needed.
 *)
...
Proof
 ...
 >> DISCH_TAC
 (* There seems no other choice for this finite map *)
