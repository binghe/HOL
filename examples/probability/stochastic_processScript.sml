...
Proof
 ...
 >- (RW_TAC fcp_ss [IN_RECTANGLE, Abbr ‘g’] \\
     Q.PAT_X_ASSUM ‘!n. n IN carrier J ==> P’
                   (MP_TAC o (Q.SPEC ‘(l :'index['N]) ' n’)) \\
 ...
 >> Q.ABBREV_TAC ‘prod = {rectangle h (:'N) |
 ...
 >- (rw [Abbr ‘g’, Abbr ‘prod’, IN_IMAGE, SUBSET_DEF] \\
    ...
    ‘count (dimindex (:'N)) <> {}’ by PROVE_TAC [COUNT_NOT_EMPTY, DIMINDEX_GT_0] \\
     ...
     MATCH_MP_TAC (REWRITE_RULE [subsets_def]
                    (Q.SPEC ‘(m_space p,measurable_sets p)’
 ...
 (* stage work *)
 >> Suff ‘IMAGE (\s. PREIMAGE g s INTER m_space p) (subsets (B of_dimension (:'N))) =
          subsets (sigma (m_space p)
                  (IMAGE (\s. PREIMAGE g s INTER m_space p) prod))’
 >- (Rewr' >> art [])
 ...
 >| [ (* goal 1 (of 2) *)
     ...
     ‘subset_class (space B) (subsets B)’
        by PROVE_TAC [sigma_algebra_def, algebra_def] \\
      FULL_SIMP_TAC std_ss [subset_class_def] \\
      METIS_TAC [SUBSET_DEF],
      (* goal 2 (of 2) *)
...
Definition consistency_condition_1_def :
   ...
   !(l :'index['N]) (h :num -> 'b set) (g :num -> num).
    ...
    ==>
      ...
      finite_dimensional_distribution p X (fcp_permutes g l)
                                          (rectangle (h o g) (:'N))
End
...
Definition consistency_condition_2_def :
   ...
   !(l :'index[1 + 'N]) h.
      ...
      (!i. i < dimindex(:'N) ==> h i IN subsets B)
    ==>
      finite_dimensional_distribution p X ((FCP_SND l) :'index['N])
                                          (rectangle h (:'N)) =
      finite_dimensional_distribution p X l
...
QED
 ...
 >| [ (* goal 1 (of 5) *)
     ...
     ‘prob p (V INTER E) = prob p V * prob p E’ by PROVE_TAC [] \\
      ...
      (* goal 4 (of 5) *)
      SIMP_TAC std_ss [PREIMAGE_BIGUNION, IMAGE_IMAGE, o_DEF,
     ...
     ‘(\x. PREIMAGE X (f x) INTER p_space p INTER E) = h’ by METIS_TAC [] \\
      POP_ORW \\
