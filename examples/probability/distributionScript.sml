...
Theorem Lipschitz_continuous_map_real :
    Lipschitz_continuous_map (extreal_mr1,mr1) real
Proof
    rw [Lipschitz_continuous_map_def, GSYM dist_def, dist]
 >> Suff ‘?k. 0 < k /\
              !x y. abs (x - y) <= k * dist extreal_mr1 (Normal x,Normal y)’
 >- (STRIP_TAC \\
     Q.EXISTS_TAC ‘k’ >> rw [] \\
     Cases_on ‘x = PosInf’ >> simp []
     >- (Cases_on ‘y = PosInf’ >> simp [MDIST_REFL] \\
        ...
        ‘?r. y = Normal r’ by METIS_TAC [extreal_cases] \\
         simp [real_normal] \\
         Q.PAT_X_ASSUM ‘!x y. abs (x - y) <= _’
           (MP_TAC o Q.SPECL [‘r’, ‘0’]) >> rw [] \\
        ‘k = k * 1’ by simp [] >> POP_ORW \\
         Q_TAC (TRANS_TAC REAL_LE_TRANS)
               ‘k * dist extreal_mr1 (Normal r,Normal 0)’ >> art [] \\
     ...
     ‘?a. x = Normal a’ by METIS_TAC [extreal_cases] \\
      ...
      >- (Q.PAT_X_ASSUM ‘!x y. abs (x - y) <= _’
         ...
         ‘k = k * 1’ by simp [] >> POP_ORW \\
          Q_TAC (TRANS_TAC REAL_LE_TRANS)
                ‘k * dist extreal_mr1 (Normal a,Normal 0)’ >> art [] \\
          MATCH_MP_TAC REAL_LE_LMUL_IMP \\
          simp [extreal_mr1_le_1, REAL_LT_IMP_LE]) \\
     ...
     ‘?b. y = Normal b’ by METIS_TAC [extreal_cases] \\
      simp [real_normal])
 (* stage work, seems impossible?! *)
 >> simp [extreal_mr1_normal]
 >> cheat
QED

(* cf. converge_in_dist_alt_continuous_on *)
Theorem converge_in_dist_alt_Lipschitz_mr1 :
    !X Y p. prob_space p /\ (!n. real_random_variable (X n) p) /\
            real_random_variable Y p ==>
           ((X --> Y) (in_distribution p) <=>
             !f. f IN BL mr1 ==>
...
Proof
 ...
 >- (qabbrev_tac ‘g = f o real’ \\
    ...
    ‘Normal o f o real o Y = Normal o g o Y’ by METIS_TAC [o_ASSOC] >> POP_ORW \\
     ...
     REWRITE_TAC [Lipschitz_continuous_map_real])
 (* stage work *)
 >> cheat
QED
