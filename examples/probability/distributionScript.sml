...
(* NOTE: This version use the open interval and asserts an real_random_variable.

   Note that the original web proof [6] indeed mentions an open interval (0,1).
   And it turns out that this setting makes the proof slightly harder.

Theorem existence_of_real_random_variable :
    !m p. prob_space (space Borel,subsets Borel,m) /\
          p = restrict_space ext_lborel {x | 0 < x /\ x < 1} ==>
          ?X. real_random_variable X p /\
              !s. s IN subsets Borel ==> distribution p X s = m s
Proof
    RW_TAC std_ss []
 >> qabbrev_tac ‘M = (space Borel,subsets Borel,m)’
 ...
 >- (simp [real_random_variable, p_space_def, events_def] \\
     ...
     >- (simp [Abbr ‘p’, space_restrict_space, sets_restrict_space] \\
         ...
         simp [GSYM restrict_algebra_def] \\
         MATCH_MP_TAC IN_MEASURABLE_BOREL_MONO_INCREASING >> rw []) \\
     simp [Abbr ‘p’, space_restrict_space, Abbr ‘sp’, ext_lborel_def, SPACE_BOREL] \\
     Q.X_GEN_TAC ‘y’ >> STRIP_TAC \\
     Q.PAT_X_ASSUM ‘!x y. x <= y ==> X x <= X y’ K_TAC \\
     simp [Abbr ‘X’, lt_infty]
     STRONG_CONJ_TAC
     >- (simp [lt_sup] \\
         cheat) \\
     cheat)
 ...
 >- (Q.PAT_X_ASSUM ‘s' IN subsets Borel’ K_TAC \\
  ...
  (* applying right_continuous_at_thm *)
     ...
     DISCH_THEN (MP_TAC o Q.SPEC ‘e’) \\
     impl_tac (* 0 < e /\ e <> PosInf *)
     >- (‘y <> NegInf’ by rw [pos_not_neginf, lt_imp_le] \\
         Know ‘y <> PosInf’
         >- (simp [lt_infty] \\
             Q_TAC (TRANS_TAC lt_trans) ‘1’ >> rw []) >> DISCH_TAC \\
         Q.PAT_X_ASSUM ‘f (Normal a) < y’ MP_TAC \\
        ‘?r. 0 < r /\ r < 1 /\ y = Normal r’
 ...
 >- (Q.PAT_X_ASSUM ‘s' SUBSET t’         K_TAC \\
     ...
     simp [Abbr ‘N’, Abbr ‘t’, Abbr ‘s'’, Abbr ‘s’, DIFF_DEF] \\
     Q.PAT_X_ASSUM ‘m_space p = sp’      K_TAC \\
     simp [GSYM CONJ_ASSOC, GSYM extreal_lt_def, Abbr ‘sp’, extreal_not_lt] \\
     simp [LEFT_AND_OVER_OR] \\
     Know ‘!x. 0 <= x /\ x <= f (Normal a) /\ 1 <= x <=> 1 <= x /\ x <= f (Normal a)’
     >- (Q.X_GEN_TAC ‘y’ \\
         EQ_TAC >> rw [] \\
         Q_TAC (TRANS_TAC le_trans) ‘1’ >> rw []) >> Rewr' \\
     Know ‘!x. 0 <= x /\ x <= f (Normal a) /\ x <= 0 <=> x = 0 /\ 0 <= f (Normal a)’
     >- (Q.X_GEN_TAC ‘y’ \\
         EQ_TAC >> rw [] \\
         rw [GSYM le_antisym]) >> Rewr' \\
     simp [] \\
     Know ‘!x. 1 <= x /\ x <= f (Normal a) <=> x = 1 /\ f (Normal a) = 1’
     >- (Q.X_GEN_TAC ‘y’ \\
         EQ_TAC >> rw []
         >- (rw [GSYM le_antisym] \\
             Q_TAC (TRANS_TAC le_trans) ‘f (Normal a)’ >> rw []) \\
         rw [GSYM le_antisym] \\
         Q_TAC (TRANS_TAC le_trans) ‘y’ >> art []) >> Rewr' \\
     qmatch_abbrev_tac ‘countable N’ \\
     Know ‘N = {x | 0 <= x /\ x <= f (Normal a) /\ (?y. f y <= x /\ Normal a < y)}
               UNION {0} UNION {x | x = 1 /\ f (Normal a) = 1}’
     >- (rw [Abbr ‘N’, Once EXTENSION] \\
         METIS_TAC []) >> Rewr' \\
     qunabbrev_tac ‘N’ \\
     qmatch_abbrev_tac ‘countable (N UNION A UNION B)’ \\
     Know ‘N = {y | 0 <= y /\ y <= f (Normal a) /\ ?x. Normal a < x /\ f x = y}’
     >- (rw [Abbr ‘N’, Once EXTENSION] >> EQ_TAC >> rw [] >| (* 2 subgoals *)
         [ (* goal 1 (of 2) *)
           Q.EXISTS_TAC ‘y’ >> art [] \\
           ...
           Q.EXISTS_TAC ‘x’ >> rw [] ]) >> Rewr' \\
     qunabbrev_tac ‘N’ \\
     qmatch_abbrev_tac ‘countable (N UNION _ UNION _)’ \\
     Know ‘N = {y | y = f (Normal a) /\ ?x. Normal a < x /\ f x = y}’
     >- (rw [Abbr ‘N’, Once EXTENSION] >> EQ_TAC >> rw [] \\
         rename1 ‘Normal a < x’ \\
         simp [GSYM le_antisym] \\
     ...
     qmatch_abbrev_tac ‘countable (N UNION _ UNION _)’ \\
     qabbrev_tac ‘z = f (Normal a)’ \\
     simp [COUNTABLE_UNION, Abbr ‘A’, COUNTABLE_SING] \\
     reverse CONJ_TAC
     >- (simp [Abbr ‘B’] \\
         Cases_on ‘z = 1’ >> rw []) \\
     Cases_on ‘?x. Normal a < x /\ f x = z’
