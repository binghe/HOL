...
ifdef(`TIMING',
`%type <node> number `expr' hexpr cexpr caselist constant ',
`%type <node> number `expr' hexpr caselist constant ')
...
spec          : PRINT hexpr {$$ = new_node(PRINT,$2,NIL);}
              | SPEC `expr' {$$ = new_node(SPEC,$2,NIL);}
              ;
...
hexpr         : HIDE netermlist COLON `expr' { $$ = new_node(HIDE,$2,$4); }
              | EXPOSE netermlist COLON `expr' { $$ = new_node(EXPOSE,$2,$4); }
              | `expr'
...
`expr'        : term
	      ...
	      | E LB `expr' BUNTIL subrange `expr' RB
              ...
              | `expr' SETNOTIN `expr'
                      { $$ = new_node(NOT,new_node(SETIN,$1,$3),NIL); }
